...
public class AvatarController : ProfileControllerBase
{
    ...
    public AvatarController(MongoPool mongoPool, IMemoryCache cache, MongoUserContext context, IAwsS3 awsS3) 
    ...
    public async Task<IActionResult> ChangeAvatar(IFormFile formFile)
    {
        ...
        var file = await GenerateUrlAsync(user, value);

        var result = await _awsS3.UploadFileAsync("users", file, formFile);
        ...
        return this.ApiResponseAccepted<AvatarDto.Response>(title: "Avatar successfully updated", 
            data: new()
            {
    ...
    private Task<string> GenerateUrlAsync(MongoUser user, string extension)
    {
        ...
        return Task.FromResult($"{builder}/avatar.{extension}");
    }
}
