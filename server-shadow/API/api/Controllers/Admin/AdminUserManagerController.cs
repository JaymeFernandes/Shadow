...
{
    [HttpGet("users")]
    public async Task<IActionResult> GetAllAsync([FromQuery] AllUserDto.Request request)
    {
        ...
        var result = new List<AllUserDto.Response.User>();
    ...
    [HttpPost("user/ban")]
    public async Task<IActionResult> BanUserAsync([FromHeader] Ban.Request request)
    {
    ...
    [HttpPost("user/unban")]
    public async Task<IActionResult> UnbanUserAsync([FromHeader] Unban.Request ban)
    {
        var actor = HttpContext.User.FindFirst(OpenIddictConstants.Claims.Subject);
        ...
        var result = await authServices.UnbanUserAsync(actor.Value, ban.Id);
